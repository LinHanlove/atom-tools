...
export class KeepLoop {
  ...
  private init(option: TYPE.IKeepLoopOption): TYPE.IKeepLoopOption {
    ...
    const params = {
      run: option.run,
      interval: option.maxCount
        ? Math.abs(parseInt(option.interval.toString(), 10))
        : 1000,
      maxCount: option.maxCount
        ? Math.abs(parseInt(option.maxCount.toString(), 10))
        : 0,
    };
  ...
  private run(): void {
    try {
      ...
      if (
        !this.isPaused &&
        (!this.option.maxCount || this.count < this.option.maxCount)
      ) {
        // 继续轮询
        this.timer = setTimeout(this.run.bind(this), this.option.interval);
      } else {
